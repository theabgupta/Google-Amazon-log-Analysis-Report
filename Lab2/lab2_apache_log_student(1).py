...
dayHostCount = (dayGroupedHosts.mapValues(f)).sortByKey()

dailyHosts = (dayHostCount.cache())
dailyHostsList = dayHostCount.take(30)
...
groupedByDay = dayAndHostTuple.groupByKey().mapValues(f).sortByKey()

sortedByDay = groupedByDay.join(dayHostCount).map(lambda (a, (b, c)): (a, b/c)).sortByKey()
...
badEndpointsTop20 = badEndpointsSum.takeOrdered(20, key=lambda (x,y): -y)
...
errHostsTop25 = errHostsSum.takeOrdered(25, key=lambda (x,y): -y)
...
errDateCountPairTuple = badRecords.map(lambda x : (x.date_time.day,1) ).groupByKey()
errDateSum = errDateCountPairTuple.mapValues(len).sortByKey()
...
topErrDate = errDateSorted.takeOrdered(5, key= lambda (x,y) : -y)
...
hourCountPairTuple = badRecords.map(lambda x:(x.date_time.hour,1)).groupByKey()

hourRecordsSum = hourCountPairTuple.mapValues(len).sortByKey()
